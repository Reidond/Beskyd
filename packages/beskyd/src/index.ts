...
export function conicalMembership(
  value: number,
  a: number,
  b: number,
  c: number
...
export function computeRegionRisk(input: ComputeRiskInput): RegionRiskResult[] {
  ...
  const expertByRegion = new Map(
    experts.map((expert) => [expert.regionId, expert.safetyLevel])
  );
  ...
  return regions.map((regionId) => {
    ...
    for (const participant of regionParticipants) {
      ...
      for (const [groupName, criteriaKeys] of Object.entries(config.criteriaGroups)) {
        ...
        if (!criteriaValues) {
          throw new Error(
            `Participant ${participant.participantId} is missing group ${groupName}.`
          );
        }
        let sum = 0;
    ...
    const deltaRisk = weightedRiskValues.reduce((sum, value) => sum + value, 0) /
    ...
    const senseOfSafety = clamp(
      1 -
        0.5 *
          Math.sqrt(
            (phiRisk - 1) * (phiRisk - 1) +
              (repeatVisitClamped - 1) * (repeatVisitClamped - 1)
          ),
      0,
      1
    );
